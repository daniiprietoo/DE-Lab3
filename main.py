...
def main():
    ...
    for record in records:
        ...
        # Insert date
        date_id = get_or_create(dim_date, record["date"], "date_id")
...
def generate_records(dicom_data: pd.DataFrame) -> list[dict]:
    records = []
...
def generate_record(dicom_file: pydicom.Dataset) -> dict:
    ...
    record = {
        ...
        "image": {
            ...
            "pixel_spacing_x": (
                normalized_pixel_spacing[0] if normalized_pixel_spacing else 0
            ),
            "pixel_spacing_y": (
                normalized_pixel_spacing[1] if normalized_pixel_spacing else 0
            ),
        ...
        "date": {
            "year": int(dicom_file.get("StudyDate", "")[:4]),
            "month": int(dicom_file.get("StudyDate", "")[4:6]),
        },
        "study": {
